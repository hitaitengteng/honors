...
# 5. File names:
#      nelson.3c.t.wy.wt.csv, e.g.
#      nelson.3c.k.ny.ny.csv, e.g.
...
install.packages('plyr', repos='http://cran.wustl.edu/')  # for mapvalues
library(plyr)             # note that this masks some dpylr methods
install.packages('dplyr', repos='http://cran.wustl.edu/')
library(dplyr)
require(dplyr)  # i don't know what this does or if it's actually needed

install.packages("gtools", repos='http://cran.wustl.edu/') # for quantcut
library(gtools)
...
############################################################
# add class attribute to tons/HA
# NOTE: my program expects a numeric value for the class attribute,
# so I have converted the H/M/L values as: "L" --> 0, "M" --> 1, "H" --> 2
class <- quantcut(dataNew2$tonsDiff, q=seq(0,1,by=1/3))
dataNew2$tHA_3 <-  mapvalues(class, from=levels(class), to=c(0, 1, 2))

############################################################
# add class attribute to BillionCals/HA
class <- quantcut(dataNew2$BcalsDiff, q=seq(0,1,by=1/3))
dataNew2$BcHA_3 <-  mapvalues(class, from=levels(class), to=c(0, 1, 2))
...
dataNew3 <- dataNew3[c(
     ...
     "navgDiff", "pavgDiff", "irrDiff", "landdDiff", "eqpDiff", "tHA_3", "BcHA_3")]
...
#write nelson.3c.t.wy.wt.csv, nelson.3c.k.wy.wt.csv


# remove id
# remove one of tons / kcals
# write to.csv file, write nelson.3c.t.wy.wt.csv, nelson.3c.k.wy.wt.csv
...
# write 8 files
write.csv(nelson.t,       'nelson.3c.t.wy.wt.csv', quote=FALSE, row.names=FALSE)
write.csv(nelson.k,       'nelson.3c.k.wy.wt.csv', quote=FALSE, row.names=FALSE)
write.csv(nelson.t.ny,    'nelson.3c.t.ny.wt.csv', quote=FALSE, row.names=FALSE)
write.csv(nelson.k.ny,    'nelson.3c.k.ny.wt.csv', quote=FALSE, row.names=FALSE)
write.csv(nelson.t.y.nt,  'nelson.3c.t.wy.nt.csv', quote=FALSE, row.names=FALSE)
write.csv(nelson.k.y.nt,  'nelson.3c.k.wy.nt.csv', quote=FALSE, row.names=FALSE)
write.csv(nelson.t.ny.nt, 'nelson.3c.t.ny.nt.csv', quote=FALSE, row.names=FALSE)
write.csv(nelson.k.ny.nt, 'nelson.3c.k.ny.nt.csv', quote=FALSE, row.names=FALSE)
...
# split all four of these into two different output files
...
# write 8 files
write.csv(nelson.t.temperate,       'nelson.3c.t.wy.temp.csv', quote=FALSE, row.names=FALSE)
write.csv(nelson.t.tropical,        'nelson.3c.t.wy.trop.csv', quote=FALSE, row.names=FALSE)
write.csv(nelson.k.temperate,       'nelson.3c.k.wy.temp.csv', quote=FALSE, row.names=FALSE)
write.csv(nelson.k.tropical,        'nelson.3c.k.wy.trop.csv', quote=FALSE, row.names=FALSE)

write.csv(nelson.t.ny.temperate,    'nelson.3c.t.ny.temp.csv', quote=FALSE, row.names=FALSE)
write.csv(nelson.t.ny.tropical,     'nelson.3c.t.ny.trop.csv', quote=FALSE, row.names=FALSE)
write.csv(nelson.k.ny.temperate,    'nelson.3c.k.ny.temp.csv', quote=FALSE, row.names=FALSE)
write.csv(nelson.k.ny.tropical,     'nelson.3c.k.ny.trop.csv', quote=FALSE, row.names=FALSE)
